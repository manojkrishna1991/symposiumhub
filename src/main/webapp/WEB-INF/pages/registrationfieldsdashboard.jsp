...
<body>
	...
	<div class="ui main container">
		<div class="ui  text container text-right">
			<a href="/step3/${eventId}" class="ui blue button">Add
				Registration Details <i class="arrow right icon"></i>
			</a>
		</div>

		<div class="ui  text container bg-background-white mtop20">
			<div class="ui segment">
				<div class="ui message">
					<div class="header">Add a registration form for your
						event.your form is auto saved</div>
					<div class="content">
						<a href="/registerForEvent/${eventId}?preview=true">preview
							your form</a>
					</div>
				</div>
			</div>
		</div>
		<div class="ui text container mtop10 bg-background-white">

			<div class="ui segment">

				<button class="ui green button addfiedbut">
					<i class="plus icon"></i>Add Field
				</button>
				<h1 style="text-align: center">Register For ${event.name}</h1>
				<form class="ui form" id="dynamicForm"
					action="/dynamicRegistrationFormHandler" method="get">
						...
						<c:forEach var="symposiumInfo"
							...
							<c:if test="${symposiumInfo.type eq 'text'}">
								...
								<div class="field">
									...
									</label><input type="text" required="required" id="fname"
							...
							<c:if test="${symposiumInfo.type eq 'textbox'}">
								...
								<div class="field">
									...
									<textarea required="required" id="fname"
										name="${symposiumInfo.name}"></textarea>
								</div>
							</c:if>
							<c:if test="${symposiumInfo.type eq 'dropdown'}">
								...
								<div class="field">
									<label style="font-size: 18px;">${symposiumInfo.name} :
										...
										style="display: none;"></i> <i style="display: none;"
										class="trash outline icon" data-id="${symposiumInfo.id}"></i>
									</label><input required="required" type="text" id="fname"
										name="${symposiumInfo.name}">
								</div>
							</c:if>
		...
		$(".addoptionform").click(function(event) {
			...
			$.ajax({
				...
				success : function(data) {
					...
					window.location.href = "/registrationfields/" + ${eventId};
